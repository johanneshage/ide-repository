...
class Main:
    def __init__(self, graph, R, ST, alpha, variante, kanten_queue, start_queue, ende_queue, y0_queue):
        """
        liest Parameter aus "data.py" ein und startet das Programm (also erzeugt eine "Application" und ruft deren
         "runner" Methode auf).
        :param graph: Gerichteter Graph als Dictionary, falls kein Graph spezifiziert, so wird Graph aus Datei
         "myGraph.gexf" eingelesen
        ...
        :param variante: gibt Variante zur Kostenberechnung an; Möglichkeiten: 'A', 'B', 'C', 'D'
    ...
    def import_gexf_graph():
        ...
        # x_max, bzw. y_max beschreiben nun die bertraglich größte x- bzw. y-Koordinate
...
def main():
    try:
        ...
        graph = None
    Main(graph,data.R,data.ST,data.alpha,data.variante,data.kanten_queue,data.start_queue,data.ende_queue,data.y0_queue)
    return 0
